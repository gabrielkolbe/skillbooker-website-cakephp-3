...
<div class="row">
  ...
  <div class="contentbox padding15">
    <?= $this->Form->create($software, ['id' => 'addsoftware']) ?>
...
<div style="display: inline-block;">
 ...
 <div style="display: inline-block;">
   ...
   <strong>Price Period</strong><BR>
       ...
       foreach($softwarePricetypes as $id => $name) {
         if($software['software_pricetype_id'] == $id) { $checked = 'checked'; } else { $checked = ''; }
        echo '<input name="software_pricetype_id" value="'.$id.'" type="radio" '.$checked.'> '.$name.' &nbsp;  ';
       }
       ?>
  <BR><BR><strong>Price Per User?</strong>
    <input name="priceperuser" value="1" type="radio" <?php if($software['priceperuser'] == 1) { echo 'checked'; } ?> > Yes &nbsp;
    <input name="priceperuser" value="0" type="radio" <?php if($software['priceperuser'] == 0) { echo 'checked'; } ?> > No
 ...
 <div class="col-md-12">
    ...
    <input name="demoavailable" value="1" type="radio" <?php if($software['demoavailable'] == 1) { echo 'checked'; } ?> > Yes &nbsp;
    <input name="demoavailable" value="0" type="radio" <?php if($software['demoavailable'] == 0) { echo 'checked'; } ?> > No
    <BR>
  ...
  <div class="col-md-12">
    ...
    <input name="freeversion" value="1" type="radio" <?php if($software['freeversion'] == 1) { echo 'checked'; } ?> > Yes &nbsp;
    <input name="freeversion" value="0" type="radio" <?php if($software['freeversion'] == 0) { echo 'checked'; } ?> > No
          <BR>
  ...
  <div class="col-md-12">
    ...
    <input name="freetrail" value="1" type="radio" <?php if($software['freetrail'] == 1) { echo 'checked'; } ?> > Yes &nbsp;
    <input name="freetrail" value="0" type="radio" <?php if($software['freetrail'] == 0) { echo 'checked'; } ?> > No
      <BR>
...
<div class="col-md-12">
  ...
  <strong>How many employees does the potential customer have?</strong><BR>
       ...
       foreach($softwareNumberemployees as $id => $name) {
        if($software['software_numberemployee_id'] == $id) { $checked = 'checked'; } else { $checked = ''; }
        echo '<input name="software_numberemployee_id" value="'.$id.'" type="radio" '.$checked.'> '.$name.' &nbsp;  ';
       }
       ?>
  </div>
<div class="col-md-12"><BR></div>
  <div style="display: inline-block;">
    <strong>How many users does the potential customer have?</strong><BR>
       ...
       if($software['software_numberuser_id'] == $id) { $checked = 'checked'; } else { $checked = ''; }
        echo '<input name="software_numberuser_id" value="'.$id.'" type="radio" '.$checked.'> '.$name.' &nbsp;  ';
       }
       ?>
  </div>
</div>
<div class="col-md-12"><BR><BR></div>
  ...
  <div class="col-md-6 col-sm-6 col-xs-6">
    <strong>How will the software be deployed?</strong><BR><BR>
       ...
       if(in_array($id,$softwarearray)) { $checked = 'checked'; } else { $checked = ''; } 
        echo '<input name="software_deployment_ids[]" id="deployed" value="'.$id.'" type="checkbox" '.$checked.' > '.$name.'<BR>';
       }
       ?>
  </div>
  <div class="col-md-6 col-sm-6">
    <strong>What support are available?</strong><BR><BR>
       ...
       if(in_array($id,$softwarearray)) { $checked = 'checked'; } else { $checked = ''; }
        echo '<input name="software_support_ids[]" value="'.$id.'" type="checkbox" '.$checked.'> '.$name.'<BR>';
       }
       ?>
    </div>
    <div class="col-md-12">.<BR><BR>.</div>
  <div class="col-md-6 col-sm-6">
    <strong>What demostration options are available?</strong><BR><BR>
       ...
       foreach($softwareDemooptions as $id => $name) {
        if(in_array($id,$softwarearray)) { $checked = 'checked'; } else { $checked = ''; }
        echo '<input name="software_demo_ids[]" value="'.$id.'" type="checkbox" '.$checked.'> '.$name.'<BR>';
       }
       ?>
  </div>
  <div class="col-md-6 col-sm-6">
    <strong>What training are provided with the price?</strong><BR><BR>
       ...
       foreach($softwareTrainings as $id => $name) {
        if(in_array($id,$softwarearray)) { $checked = 'checked'; } else { $checked = ''; }
        echo '<input name="software_training_ids[]" value="'.$id.'" type="checkbox" '.$checked.'> '.$name.'<BR>';
       }
       ?>
  </div>
  ...
  <div class="col-md-12"><BR><BR><span class="large-legend">Description</span><BR><BR></div>
    ...
    <?php if(!empty($software['software_features'])) { 
       ...
       foreach($softwareFeatures as $id => $name) {
        if(in_array($id,$softwarearray)) { $checked = 'checked'; } else { $checked = ''; }
        echo '<input name="software_feature_ids[]" value="'.$id.'" type="checkbox" '.$checked.'> '.$name.'<BR>';
       }
