...
<div class="row contentbox">
  ...
  <h2><?= h($timesheet->name) ?></h2> <BR>
    ...
    <h3><strong>Employee:</strong> <?= h($employee) ?></h3> <BR><BR>
    
<?php echo $this->Form->create(null,  ['url' => ['plugin' => null,'controller' => 'timesheets','action' => 'approveaction'], 'type' => 'file']); ?>
  <?php echo $this->Form->hidden('string', ['value'=>$string]); ?>
  <?php //echo $this->Form->input('signature', ['class'=>'form-control', 'label' => 'Please upload your signature', 'type' => 'file']); ?>
  
  <BR>
  <strong>I confirm the timesheet is correct </strong> <?php echo $this->Form->checkbox('confirm');  ?>
  <BR>
  <?= $this->Form->button(__('Approve'), ['class'=>'btn btn-primary']) ?>
  <?= $this->Form->end() ?>
