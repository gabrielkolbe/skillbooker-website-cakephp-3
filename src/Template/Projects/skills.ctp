...
<div class="row">
<div class="col-md-12">
        <h2>Skills for project <?= $project->name ?></h2>

    <?= $this->Form->create() ?>
    <?= $this->Form->button(__('Submit'), ['class'=>'btn btn-primary float-right']) ?>
    
    <h4>Skills will help us connect you to the right freelancer</h4>
    ...
    foreach($skills as $key => $name){
      if (in_array($name, $projectskills)) { echo '<option value="'.$key.'" selected>'.$name.'</option>'; } else  echo '<option value="'.$key.'">'.$name.'</option>'; 
     }
    
    ?>
     </select>
     <BR><BR>
    <h4>Not in the list?, add it here</h4>
    <?php echo $this->Form->input('skilllist', ['class'=>'form-control', 'label' => false, 'placeholder'=>'Comma seperated list', 'value' => $project->skills]); ?>
   <BR>
    <?= $this->Form->button(__('Submit'), ['class'=>'btn btn-primary float-right']) ?>    
    <?= $this->Form->end() ?>
  </div>
