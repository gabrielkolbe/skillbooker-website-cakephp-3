...
class SoftwaresController extends AppController
{
      ...
      public function beforeFilter(Event $event)
    {
        ...
        $this->set('pagetitle', 'Skillbooker.com -  Software Market');
        $this->set('taglist', 'Software Market');
        $this->set('pagedescription', 'Software Market');  
        ...
        public function index()
    {
        ...
        $session = $this->request->session();
      	...
      	if($session->check('softwaresearch')){
              $search = $session->read('softwaresearch');
      	}
        
        if ($this->request->is('post')) {
        if($this->request->data['sendfrom'] == 'softwaresearch'){
          ...
          $this->paginate = [
              'join' => [
                  [
                    ...
                    'alias' => 'SoftwareCategories',
                    'type' => 'LEFT',
                    'conditions' => [
                     'SoftwareCategories.id = Softwares.software_category_id'
                    ]
                  ]
                ],
                'conditions' => ['Softwares.status'=>1,
                'OR' => [
                  'Softwares.name LIKE' => "%$search%",
                  'Softwares.long_description LIKE' => "%$search%",                                   
                  'SoftwareCategories.name LIKE' => "%$search%",                                                     
                ]
                ]
                    
            ];
            
        $session->write('softwaresearch',$search);
      ...
      public function category($slug)
    {
        ...
        if($this->request->data['sendfrom'] == 'narrowsearch') {
        
        if(!empty($this->request->data['feature_ids'])) {
            $feature_ids = $this->request->data['feature_ids'];
        ...
        $softwarefeaturecondition = '';
        
        //SELECT FIND_IN_SET('y','x,y,z'); -- 2
        if(!empty($feature_ids)) {
        
        $result = '';
        foreach($feature_ids as $key => $value) {
          $result .= $value.',';
          $softwarefeaturecondition .= ' FIND_IN_SET('.$value.',Softwares.software_features) AND';
        }
        $result =  rtrim($result, ',');
        $selectfeatures = $feature_ids;
        
        $softwarefeaturecondition = rtrim($softwarefeaturecondition, 'AND');
        
        //dd($softwarefeaturecondition);        
          ...
          $this->paginate = [
              'join' => [
                ...
                'conditions' => ['SoftwareCategories.slug'=>$categoryslug,  $softwarefeaturecondition ]
        ...
        if(!empty($slug)) {
            ...
            $this->paginate = [
            'conditions' => ['SoftwareCategories.slug' => $slug],
            'join' => [
                  'table' => 'software_categories',
                  'alias' => 'SoftwareCategories',
                  'type' => 'LEFT',
            ...
            $this->Flash->error(__('Sorry no category available.'));
        ...
        $category = $this->SoftwareCategories->find('all', ['conditions' => ['SoftwareCategories.slug' => $slug]]);
        $category = $category->first();
        $categoryname = $category->name;
        $categoryslug = $category->slug;
        $this->set('categoryname', $categoryname);
        $this->set('categoryslug', $categoryslug);
        ...
        $this->loadModel('SoftwareCategories');
        $categories = $this->SoftwareCategories->find('all');
        
        $this->loadModel('SoftwareFeatures');
        $features = $this->SoftwareFeatures->find('all', ['conditions' => ['SoftwareFeatures.software_category_id' => $category->id]]);
        
        $this->set('selectfeatures',$selectfeatures); 
        $this->set(compact('softwares', 'categories', 'features'));
        $this->set('_serialize', ['softwares']);
        
        $this->set('pagetitle', 'Skillbooker.com -  '.$categoryname);
        $this->set('taglist', $categoryname);  
    ...
    public function lists()
    {
        ...
        if($user_id == 1) {} else {
          ...
          $this->paginate = [
              'conditions' => ['Softwares.user_id = '.$user_id],   
          ];
    ...
    public function view($slug = null)
    {
        ...
        $this->loadModel('SoftwareCategories');
        $category = $this->SoftwareCategories->get($software->software_category_id);
        ...
        $this->set('pagedescription', $software->short_description);
        
                
        $this->set('pagetitle', $software->name.' - Skillbooker.com');
        $this->set('taglist', $software->name);  

        $this->set(compact('software', 'softwareDeployments', 'softwareSupports', 'softwareTrainings', 'softwareFeatures',  'softwareDemooptions', 'priceperiod', 'currency', 'category'));
    ...
    public function add()
    {
        ...
        if( $credit->software < 1) {
          //buy credit
          $this->Flash->success(__('Please buy some credit.'));
          return $this->redirect(['plugin' => null, 'controller' => 'salesoptions', 'action' => 'software']);
        }
   ...
   /*         
          ...
          public function getcategory($slug)
    {
        ...
        $this->paginate = [
            'conditions' => ['SoftwareCategories.name LIKE ' => "$slug%"],
        ];
        ...
        $this->set('_serialize', ['categories']); 
            ...
            $this->Flash->error(__('Sorry no category available under that letter.'));
            return $this->redirect(['plugin' => null, 'controller' => 'softwares', 'action' => 'index']);
        }
