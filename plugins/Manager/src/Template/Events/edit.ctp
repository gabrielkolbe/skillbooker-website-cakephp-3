   ...
   <?= $this->Form->create($event, ['enctype' => 'multipart/form-data']) ?>
    <fieldset>
        ...
        <?php
            ...
            echo '<BR><strong>Select the main image for this event </strong><BR><BR>';
            foreach($images as $image){

              echo '<input type="radio" name="mainimage" value="'.$image->name.'">';
              echo $this->Html->image('small_'.$image->name, ['class'=> 'thumbnailsmall']);
              echo '<BR>';
            }
        ?>
    </fieldset>
