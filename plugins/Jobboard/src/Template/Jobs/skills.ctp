...
<?= $this->Html->link(__('Back -> My Jobs'), ['plugin' => 'jobboard', 'controller' => 'jobs', 'action' => 'jobs'], ['class' => 'btn btn-primary btn-xs float-right']) ?>
<div class="row">
<div class="col-md-12">
    ...
    foreach($skills as $key => $name){
      if (in_array($name, $jobskills)) { echo '<option value="'.$key.'" selected>'.$name.'</option>'; } else  echo '<option value="'.$key.'">'.$name.'</option>'; 
     }
